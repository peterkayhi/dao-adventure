...
actor {
        ...
        stable let canisterIdWebpage : Principal = Principal.fromText("4k2wq-cqaaa-aaaab-qac7q-cai");
        ...
        public shared func getBalance(p:Principal) : async Nat { // mine
                let tempNat : Nat = await mbtCanister.balanceOf(p);
                return tempNat;
        };
        ...
        // mine: returns identity of caller
        ...
        public shared func addMember(p : Principal, name : Text) : async Result<(), Text> {
                switch (members.get(p)) {
                        case (null) { // not found, add new member
                                ...
                                return (await mbtCanister.mint(p,10) );  // Airdrop 10 MBC tokens to the new member and return passed Result
                                return #ok();
                        };
                        case (?member) {
                                return #err("Member already exists");
        ...
        public shared func mbtEdit (p : Principal, qty: Int) : async Result<Nat, Text> {
                var tempNat : Nat = 0;
                if (qty <= 0) { // negative or zero, therefore subtract (burn)
                        let burnVar = await mbtCanister.burn(p, Int.abs(qty)); // take abs of qty b/c you can only burn a Nat number
                        switch (burnVar) { // did it work?
                                case (#err(mbtBurnErrorText)) { // no burn
                                        return #err(mbtBurnErrorText);
                                };
                                case (#ok) { // it worked - now get the balance
                                        tempNat := await mbtCanister.balanceOf(p); // get the balance
                                };

                        };
                } else { // qty is positive, mint (add)
                        let mintVar = await mbtCanister.mint(p, Int.abs(qty)); //mint wants a Nat so take the abs of the Int qty
                        switch (mintVar) { // did it work?
                                case (#err(mbtMintErrorText)) { //no can mint
                                        return #err(mbtMintErrorText);
                                };
                                case (#ok) {// it worked - now get the balance
                                        tempNat := await mbtCanister.balanceOf(p); // get the balance
                                };
                ...
                return #ok(tempNat); // return the mbt balance
        };
        ...
        public query func getAllMembers() : async Text {
                var allMembers : Text = "";
                for ((key, value) in members.entries()) {
                        allMembers := Text.concat (allMembers, "(" # Principal.toText(key) # "," );
                        allMembers := Text.concat (allMembers, value.name # ")" # "\u{000D}" # "\u{000A}"); //append cr/lf
                ...
                switch (content) {
                        ...
                        case (#AddMentor(newMentor)) { //graduate member to mentor. newMentor holds principal (index) of member
                                switch (members.get(newMentor)) {
                                        case (null) {}; // not found - should not happen but I can't return an error here
                        ...
                        case (_) {};  // don't care about other cases - probably remove #AddGoal as content type
                };
